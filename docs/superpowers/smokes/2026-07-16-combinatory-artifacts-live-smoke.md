...
1. **Cold-start / reconnect first-typed-message drop, ×2.** On both a cold session start and an auto-restart after a link drop, the first typed message got a generic "Please tell me what you would like to do" AND a spurious `teach_sequence` overlay (numbered badges on program chrome). The earlier pendingTypedRef fix may not cover reconnect ordering (typed text vs. context hints). Needs investigation.
