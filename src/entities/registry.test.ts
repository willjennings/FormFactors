...
describe('buildEntities', () => {
  ...
  it('merges sub-entities (cells) for excel', () => {
    const es = buildEntities(excel, doc, {}, null);
    const subs = es.filter(e => e.sub);
    expect(subs.length).toBeGreaterThan(0);
    expect(subs.some(e => e.id === 'excel-cell-A1')).toBe(true);
  });
});
...
describe('resolveEchoedTarget', () => {
  ...
  it('resolves exact and containment matches on titles', () => {
    expect(resolveEchoedTarget(es, 'Spreadsheet grid')!.entity.title).toBe('Spreadsheet grid');
  ...
  it('resolves via perceived-label aliases (the G5 fix)', () => {
    expect(resolveEchoedTarget(es, 'grid of numbers')!.entity.title).toBe('Spreadsheet grid');
  });
  it('REGRESSION (session 2026-07-02): "Cell A3" resolves to the cell sub-entity, not the top-level grid tile', () => {
    expect(resolveEchoedTarget(es, 'Cell A3')?.entity.id).toBe('excel-cell-A3');
  });
