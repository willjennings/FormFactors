...
export interface ScorecardModel {
  ...
  latency: {
    ...
    // transcript, or `opts.events` is empty).
...
const roundCount = (rate: { value: number; n: number }): number => Math.round(rate.value * rate.n);
...
/** Same "lower median" rule as armAggregate.ts's own `lowerMedian` (not re-exported from there —
 *  duplicating the four-line function rather than reaching into a sibling module's private helper
 *  is the smaller coupling; the RULE, not the code, is what has to match, and it is stated
 *  identically here). Returns `null` for an empty input, never 0. */
...
function buildLatency(events: TelemetryEvent[]): ScorecardModel['latency'] {
  ...
  return { medianMs, worst, coldStartMs };
}
...
export function scorecard(agg: ArmAggregate, ledger: LedgerRow[], deck: CardResult[], arm: Arm, opts: ScorecardOpts): ScorecardModel {
  ...
  const refusedCount = roundCount(agg.refusal);
  if (refusedCount > 0) goodAt.push(withN('honest refusals', refusedCount, agg.n));

  // Corrections / undone-after-landing — shaky, not good, not a refusal-shaped failure either.
  const correctedCount = roundCount(agg.corrected);
  if (correctedCount > 0) shaky.push(withN('corrected mid-attempt', correctedCount, agg.n));
  const wrongCount = roundCount(agg.wrong);
  if (wrongCount > 0) shaky.push(withN('undone after landing', wrongCount, agg.n));
  ...
  // Watch: no-op turns (capabilityLedger's 'no-op-turn' rows) — ALWAYS rendered, at any count,
  // never dropped (the speech_only binding rule). Each row already carries its own n and a
  // verbatim first-seen example.
  for (const row of ledger) {
    if (row.kind !== 'no-op-turn') continue;
  ...
  const ungradeableCount = roundCount(agg.ungradeable);
  if (agg.n > 0 && agg.ungradeable.value > UNGRADEABLE_UNRELIABLE_SHARE) {
    watch.push(withN(
      ...
      ungradeableCount, agg.n,
    ));
  }

  // Watch: the partial-recording marker (Task 7) — rendered only when non-zero.
  if (opts.unrecorded && opts.unrecorded > 0) {
    const n = opts.unrecorded;
    watch.push(withN(
      `${n} card result${n === 1 ? '' : 's'} recorded in this panel only — the recorder was off`,
  ...
  const t = deckTally({ index: 0, results: deck, startedAt: deck.length ? 0 : null });
