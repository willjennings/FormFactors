...
export const ACTION_VERBS: Partial<Record<ActionCategory, ActionVerb>> = {
  ...
  insert: {
    ...
    // "in a spreadsheet" is load-bearing: this one description is handed to every program that
    // exposes insert_object, which includes PowerPoint, where there are no columns to total.
    description: 'Insert a new object (a chart, a new slide, a shape), or — in a spreadsheet — compute a column aggregate (sum/total, average) into the first free cell below that column. HIGH-COMMITMENT — it adds to the document. Witness-render first, then commit with confirm=true.',
    parameters: VERB_PARAMETERS,
  },
  photo: {
