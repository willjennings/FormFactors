...
import type { MockDoc, Program, ProgramId, ProgramImage } from '../scenarios';
...
// DOM node stamped data-entity-id (the generic measurement contract) so teaching overlays
...
export function SurfaceElement({ img, programId, live, focusTitle, blocked, onElementClick, className, children }: {
  img: ProgramImage; programId: ProgramId; live: boolean; focusTitle?: string; blocked?: boolean;
  ...
  return (
    <div
      data-entity-id={`${programId}-${img.id}`}
...
function WordSurface({ program, doc, live, focusTitle, blockedElements, onAction, onElementClick }: SurfaceProps) {
  ...
  return (
    <div className="flex flex-col h-full gap-2">
      <SurfaceElement img={imgOf(program, 1)} programId={program.id} live={live} focusTitle={focusTitle} blocked={blockedElements?.includes(1)} onElementClick={onElementClick}
        ...
        <SurfaceElement img={imgOf(program, 2)} programId={program.id} live={live} focusTitle={focusTitle} blocked={blockedElements?.includes(2)} onElementClick={onElementClick}>
          <RibbonButton icon={<Save size={16} />} label="Save"
            onClick={() => onAction('save_file', { target: 'Save button' })} />
        </SurfaceElement>
        <SurfaceElement img={imgOf(program, 3)} programId={program.id} live={live} focusTitle={focusTitle} blocked={blockedElements?.includes(3)} onElementClick={onElementClick}>
          <RibbonButton icon={<SaveAll size={16} />} label="Save As"
            onClick={() => onAction('save_file', { target: 'Save As button', detail: 'Save As' })} />
        </SurfaceElement>
      </SurfaceElement>
      <SurfaceElement img={imgOf(program, 4)} programId={program.id} live={live} focusTitle={focusTitle} blocked={blockedElements?.includes(4)} onElementClick={onElementClick}
...
function ExcelSurface({ program, doc, live, focusTitle, blockedElements, onAction, onElementClick }: SurfaceProps) {
  ...
  return (
    <div className="flex flex-col h-full gap-2">
      ...
      <SurfaceElement img={imgOf(program, 4)} programId={program.id} live={live} focusTitle={focusTitle} blocked={blockedElements?.includes(4)} onElementClick={onElementClick}
        className="flex-1 rounded-lg border border-[var(--card-border)] overflow-hidden">
        <Spreadsheet doc={doc} entityIdFor={(ref) => `${program.id}-cell-${ref}`} onCellClick={undefined} />
      </SurfaceElement>
    </div>
  );
}
