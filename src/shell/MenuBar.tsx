...
export function MenuBar({ isLive, isConnecting, isDarkMode, traffic, registerLabel, registerGlyph, onToggleTheme, onToggleDrawer, onRambleMode, onSketchBoard, onMissions, onRegisterPill, lead, trail, frameClass, skinKey }: MenuBarProps & {
  ...
  return (
    <div className={`absolute top-0 left-0 right-0 z-30 flex items-center justify-between gap-3 px-4 h-12 ${frameClass ?? 'border-b border-[var(--card-border)] bg-[var(--card-bg)]/80 backdrop-blur'}`} data-shell data-shell-skin={skinKey} onPointerDown={(e) => e.stopPropagation()}>
      ...
      <div className="flex items-center gap-3 min-w-0">
        ...
        <button data-register-pill onClick={onRegisterPill} className="hit-24 shrink-0 flex items-center gap-1 rounded-full border border-[var(--card-border)] px-2 py-0.5 text-[11px] font-medium text-[var(--text-primary)] hover:bg-[var(--bg-color)]" aria-label={`Register: ${registerLabel} — open register band`}>
