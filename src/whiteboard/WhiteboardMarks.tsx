...
import { seedFrom } from '../ink/rough';
import { inkLine, inkRect, inkEllipse, inkArrowhead } from '../ink/stroke';
...
export function WhiteboardMarks({ state }: { state: WhiteboardState }) {
  ...
  return (
    <svg ref={svgRef} className="absolute inset-0 w-full h-full overflow-visible pointer-events-none" viewBox="0 0 100 100" preserveAspectRatio="none">
      {/* connectors first (under nodes) — ink v2: variable-width filled polygons */}
      {state.marks.map((m) => {
        ...
        return (
          <g key={m.id} fill={INK}>
            <path d={inkLine(x1, y1, x2, y2, seedFrom(m.id), { aspect })} />
            <path d={inkArrowhead(x2, y2, angle, seedFrom(m.id + '/head'), { aspect })} />
          </g>
        );
      })}
      {nodes.map((n) => {
        ...
        return (
          <g key={n.key}>
            {/* crisp fill inset behind the ink outline keeps text readable over other marks */}
            {n.shape === 'ellipse'
            ...
            <path
              d={n.shape === 'ellipse'
                ? inkEllipse(x + w / 2, y + h / 2, w / 2, h / 2, seedFrom(n.key), { aspect })
                : inkRect(x, y, w, h, seedFrom(n.key), { aspect })}
              fill={INK} />
